...
export default {
  ...
  computed: {
    ...mapState({
      initialValues: state => state.certificate.certificateDetail,
      options: state => ({})
    })
  },
  methods: {
    ...mapActions(['getCertificateDetail', 'postCertificateUpdate']),
    onSubmit: function(values) {
      this.postCertificateUpdate(values).then(res => {
        if (res.success) {
          this.$router.back()
